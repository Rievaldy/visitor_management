                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-6 text-left">
                            <h4 class="box-title">Area List</h4>
                        </div>
                        <div class="col-6 text-right">
                            <a href="{{ route('areaAdd') }}" class="btn btn-bold btn-pure btn-info">Add New Location</a>
                        </div>
                    </div>
                </div>
                <div class="box-body">
                        ...
                        <table id="data-tables" class="table table-striped table-bordered" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Confidential Category</th>
                                    <th class="text-center">Risk Work</th>
                                    <th class="text-center">Risk Not Work</th>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="box">
                                <div class="box-header">
                                    <h4 class="box-title">View Area</h4>
                                </div>
                                ...
                                @csrf
                                    <div class="box-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="area_name" class="form-control" value="" readonly>
                                        </div>
                                        <div class="form-group">
                                            <label>Confidential Category</label>
                                            <input name="cat_confidential" type="text" class="form-control" readonly>
                                        </div>
                                        <div class="form-group">
                                            <label>Risk Work</label>
                                            <input name="risk_work" type="text" class="form-control" readonly>
                                        </div>
                                        <div class="form-group">
                                            <label>Risk Not Work</label>
                                            <input name="risk_not_work" type="text" class="form-control" readonly>
                                        </div>                                 </div>
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            colorBtnCategory = ['btn-success','btn-info','btn-warning'];
                ...
                columnDefs: [
                    {  targets: 4,
                        render: function (data, type, row, meta) {
                            if(type === 'display'){
                                console.log(data);
                                return '<span class="btn btn-block btn-rounded '+colorBtnCategory[data.cat_confidential-1]+' ">'+
                                    data.display+
                                    '</span>';
                            }else{
                                return data.cat_confidential;
                            }
                        },
                    },
                    {  targets: 5,
                        render: function (data, type, row, meta) {
                            if(type === 'display'){
                                return '<span class="btn btn-block btn-rounded '+colorBtnCategory[data.risk_work-1]+' ">'+
                                    data.display+
                                    '</span>';
                            }else{
                                return data.risk_work;
                            }
                        },
                    },
                    {  targets: 6,
                        render: function (data, type, row, meta) {
                            if(type === 'display'){
                                return '<span class="btn btn-block btn-rounded '+colorBtnCategory[data.risk_not_work-1]+' ">'+
                                    data.display+
                                    '</span>';
                            }else{
                                return data.risk_not_work;
                            }
                        },
                    },
                    {  targets: 8,
        ...
        $('#data-tables').on('click', 'button.btn-view', function () {
            ...
            $.ajax({
                ...
                success: function(item) {
                    ...
                    $('#modal-detail').find('input[name=area_name]').val(item.area_name)
                    $('#modal-detail').find('input[name=cat_confidential]').val(item.cat_confidential.display)
                    $('#modal-detail').find('input[name=risk_work]').val(item.risk_work.display)
                    $('#modal-detail').find('input[name=risk_not_work]').val(item.risk_not_work.display)
                    // console.log(item.id)
                    $('#modal-detail').find('form').attr('action','/areas/edit/'+item.id)
